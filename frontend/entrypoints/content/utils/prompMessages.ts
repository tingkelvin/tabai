...
export class PromptBuilder {
     ...
     */
    static buildMessage(config: PromptConfig): string {
        const { useAgent, task, userMessage, fileContent, pageState } = config;
        ...
        if (useAgent && pageState) {
            ...
            message += this.getAgentInstructions();
        }
     ...
     */
    static buildContinuationMessage(task: string, fileContent: string, pageState: PageState): string {
        let message = `<task>Page updated. Continue with task: ${task}</task>`;
        message += `<context>${fileContent}</context>`;
        message += `<page_state>${pageState.domSnapshot?.root.clickableElementsToString()}</page_state>`;
        message += this.getAgentInstructions();
...
Return only JSON with actions and reasoning, do not include any other text:
{
  ...
  "reasoning": "Brief explanation of the action sequence"
     ...
     */
    static parseAgentResponse(response: string): AgentResponse | null {
        try {
     ...
     */
    static getSystemPrompt(useAgent: boolean = false, useSearch: boolean = false): string {
...
Always respond with valid JSON containing actions and reasoning.`;
    ...
    /**
     * Create a debug message for development
     */
    static createDebugMessage(config: PromptConfig): string {
        return `
DEBUG INFO:
- Mode: ${config.useAgent ? 'Agent' : 'Chat'}
- Search: ${config.useSearch ? 'Enabled' : 'Disabled'}
- Has File Content: ${!!config.fileContent}
- Has Page State: ${!!config.pageState}
- Task: ${config.task || 'None'}
- User Message: ${config.userMessage || 'None'}
`;
    }
}
