import { CoordinateSet, HashedDomElement, ViewportInfo } from "./views";

export abstract class BaseDomNode {
    isVisible: boolean;
    parent: ElementDomNode | null;

    constructor(isVisible: boolean, parent?: ElementDomNode | null) {
...
export class TextDomNode extends BaseDomNode {
    type = 'TEXT_NODE' as const;
    text: string;

    constructor(text: string, isVisible: boolean, parent?: ElementDomNode | null) {
...
export class ElementDomNode extends BaseDomNode {
    tagName: string | null;
    /**
    ...
    getAllTextTillNextClickableElement(maxDepth = -1): string {
        ...
        const collectText = (node: BaseDomNode, currentDepth: number): void => {
            if (maxDepth !== -1 && currentDepth > maxDepth) {
                return;
            }

            // Skip this branch if we hit a highlighted element (except for the current node)
            if (node instanceof ElementDomNode && node !== this && node.highlightIndex !== null) {
                return;
            }

            if (node instanceof TextDomNode) {
                textParts.push(node.text);
            } else if (node instanceof ElementDomNode) {
    ...
    clickableElementsToString(includeAttributes: string[] = []): string {
        ...
        const processNode = (node: BaseDomNode, depth: number): void => {
            ...
            } else if (node instanceof TextDomNode) {
    ...
    getFileUploadElement(checkSiblings = true): ElementDomNode | null {
        ...
        if (checkSiblings && this.parent) {
            for (const sibling of this.parent.children) {
                if (sibling !== this && sibling instanceof ElementDomNode) {
