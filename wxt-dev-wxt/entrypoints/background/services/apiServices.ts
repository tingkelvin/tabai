...
const handleApiResponse = async (response: Response, errorContext: string = "API request"): Promise<ApiResponse> => {
  ...
  if (!response.ok) {
    ...
    const error: ApiError = new Error(`${errorContext} failed: ${errorDetail}`);
  ...
  if (!data || !data.reply) {
    const error: ApiError = new Error(`No response from ${errorContext}`);
...
export const chatWithLlm = async (message: string, appToken: string): Promise<ApiResponse> => {
  const payload = {
    message: message,
  };
...
export const chatWithUpload = async (
  ...
  formData.append("use_search", String(options.useSearch || false));
...
export const verifyGoogleToken = async (idToken: string): Promise<AuthResponse> => {
...
export const verifyGoogleAccessToken = async (token: string): Promise<AuthResponse> => {
